...
//#region node_modules/.nitro/vite/services/ssr/assets/router-CPSvef94.js
...
var ALLOWED_ORIGINS$2 = ["https://wolvcapital.com", "https://www.wolvcapital.com"];
function getCorsHeaders$2(request) {
	const origin = request.headers.get("origin");
	return {
		"Access-Control-Allow-Origin": ALLOWED_ORIGINS$2.includes(origin || "") ? origin : "https://wolvcapital.com",
...
function buildWidgetSource$1(apiBase) {
  ...
  var state = { visitor: null, conversation: null, sb: null, channel: null, opened: false, initPromise: null };
  ...
  async function send() {
    var v = ipt.value.trim(); if (!v) return;
    if (!state.conversation) {
      try { await init(); } catch (e) {}
    }
    if (!state.conversation) {
      append({ id:'err_'+Date.now(), role:'system', content:'Chat is still starting. Please try again in a moment.' });
      return;
    }
    ...
    try {
      ...
      showTyping(false);
      append({ id:'err_'+Date.now(), role:'system', content:'Message failed to send. Please try again.' });
    }
  }
...
var Route$6 = createFileRoute("/widget.js")({ server: { handlers: {
	OPTIONS: async ({ request }) => {
		return new Response(null, {
			status: 204,
			headers: getCorsHeaders$2(request)
		});
	},
	GET: async ({ request }) => {
		const corsHeaders = getCorsHeaders$2(request);
		const origin = new URL(request.url).origin;
		return new Response(buildWidgetSource$1(origin), { headers: {
			...corsHeaders,
...
var ALLOWED_ORIGINS$1 = ["https://wolvcapital.com", "https://www.wolvcapital.com"];
function getCorsHeaders$1(request) {
	const origin = request.headers.get("origin");
	return {
		"Access-Control-Allow-Origin": ALLOWED_ORIGINS$1.includes(origin || "") ? origin : "https://wolvcapital.com",
...
function sb() {
	return createClient(process.env.WOLV_SUPABASE_URL, process.env.WOLV_SUPABASE_SERVICE_ROLE_KEY, { auth: {
		storage: void 0,
...
function parseUA(ua) {
	return {
		...
		os: /Windows/.test(ua) ? "Windows" : /Mac OS X/.test(ua) ? "macOS" : /Android/.test(ua) ? "Android" : /iPhone|iPad/.test(ua) ? "iOS" : /Linux/.test(ua) ? "Linux" : "Other"
	};
}
...
async function geoLookup(ip) {
	...
	try {
		const r = await fetchWithTimeout(`https://ipapi.co/${ip}/json/`, 2500);
		if (!r.ok) return {};
...
var Route$1 = createFileRoute("/api/public/widget")({ server: { handlers: {
	OPTIONS: async ({ request }) => {
		return new Response(null, {
			status: 204,
			headers: getCorsHeaders$1(request)
		});
	},
	GET: async ({ request }) => {
		const corsHeaders = getCorsHeaders$1(request);
		const origin = new URL(request.url).origin;
		return new Response(buildWidgetSource(origin), { headers: {
			...corsHeaders,
			"Content-Type": "application/javascript; charset=utf-8",
			"Cache-Control": "public, max-age=300"
		} });
		...
		try {
			switch (payload.action) {
				case "init": {
					...
					const { data: existing } = await supa.from("visitors").select("*").eq("session_id", payload.session_id).maybeSingle();
					let visitor = existing;
					if (!visitor) {
						const geo = await geoLookup(ip);
						const { data: created, error } = await supa.from("visitors").insert({
						...
						visitor = created;
					} else await supa.from("visitors").update({
						last_seen: (/* @__PURE__ */ new Date()).toISOString(),
						...payload.current_page ? { current_page: payload.current_page } : {}
					}).eq("id", visitor.id);
					if (payload.current_page) await supa.from("page_views").insert({
						visitor_id: visitor.id,
						url: payload.current_page,
						referrer: payload.referrer
					});
					const { data: conv } = await supa.from("conversations").select("*").eq("visitor_id", visitor.id).neq("status", "closed").order("created_at", { ascending: false }).limit(1).maybeSingle();
					let conversation = conv;
					if (!conversation) {
						const { data: c, error } = await supa.from("conversations").insert({ visitor_id: visitor.id }).select().single();
						if (error) throw new Error(error.message);
						conversation = c;
					}
					return Response.json({
						...
						conversation,
						supabase: {
							url: process.env.WOLV_SUPABASE_URL,
							anon: process.env.WOLV_SUPABASE_PUBLISHABLE_KEY
						}
					}, { headers: corsHeaders });
				}
				case "history": {
					const { data: messages, error } = await supa.from("messages").select("*").eq("conversation_id", payload.conversation_id).order("created_at", { ascending: true });
					if (error) throw new Error(error.message);
					return Response.json({ messages: messages || [] }, { headers: corsHeaders });
				}
				case "send": {
					const { data: msg, error } = await supa.from("messages").insert({
						conversation_id: payload.conversation_id,
						role: "visitor",
						content: payload.content
					}).select().single();
					if (error) throw new Error(error.message);
					return Response.json({ message: msg }, { headers: corsHeaders });
				}
				case "human":
					await supa.from("conversations").update({ status: "human" }).eq("id", payload.conversation_id);
				...
				case "track": {
					const { data: v } = await supa.from("visitors").select("id").eq("session_id", payload.session_id).maybeSingle();
					if (!v) return Response.json({ ok: false }, { headers: corsHeaders });
				...
				case "sign_upload": {
					const bucket = supa.storage.from("chat-attachments");
					const filePath = `${payload.conversation_id}/${Date.now()}_${payload.filename}`;
					const { data, error } = await bucket.createSignedUploadUrl(filePath);
					if (error) throw error;
					const readUrl = `${process.env.WOLV_SUPABASE_URL}/storage/v1/object/public/chat-attachments/${filePath}`;
					return Response.json({
						upload_url: data.signedUrl,
						read_url: readUrl
					}, { headers: corsHeaders });
				}
				default: return Response.json({ error: "Unknown action payload received" }, {
					status: 400,
					headers: corsHeaders
				});
			}
			...
			console.error("Widget API execution failed:", errMsg);
...
var ALLOWED_ORIGINS = ["https://wolvcapital.com", "https://www.wolvcapital.com"];
function getCorsHeaders(request) {
	const origin = request.headers.get("origin");
	return {
		"Access-Control-Allow-Origin": ALLOWED_ORIGINS.includes(origin || "") ? origin : "https://wolvcapital.com",
